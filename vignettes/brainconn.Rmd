...
output: 
  html_document:
    keep_md: true
vignette: >
  %\VignetteIndexEntry{Plotting tool for brain connectivity data}
...
The minimum user input requirements for the `brainconn()` function is a connectivity matrix. This can be read into r as a matrix or data.fame. Some example connectivity matrices can be found in the `data/example` folder. The number of rows/columns should match the number of regions in the selected atlas. The atlases currently included with in the package are: `aal116`, `aal90`, `craddock200`, `dk68`, `dkt60`, `schafer300_n7`. Custom atlases can be added easily in `.csv` format (see adding custom atlas section). 
...
brainconn(atlas ="Stages_melbBrain", conmat=x, node.size = 8)
```
...
```{r, out.width='50%', warning=FALSE, message=FALSE}
x <- read.csv("data/example/example_weighted_undirected.txt", header = F)

brainconn(atlas ="schafer300_n7", conmat=x, node.size = 7,view="bottom", scale.edge.width = c(1,6), show.legend = F)
```
...
```{r, out.width='50%', warning=FALSE, message=FALSE}
x <- read.csv("data/example/example_unweighted_directed.txt", header = F)

brainconn(atlas ="schafer300_n7", conmat=x, node.size = 4, view="right", edge.alpha = 1)
```

Weighted and directed matrix example:
```{r, out.width='50%', warning=FALSE, message=FALSE}
x <- read.csv("data/example/example_weighted_directed.txt", header = F)
